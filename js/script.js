(() => {
  ...
  function drawContent(t) {
    ...
    // the polaroid itself, scaling/fading in
    const pStack = progressBetween(t, 0.0, 0.2);
    if (pStack <= 0) return;
    ...
    ctx.save();
    ctx.globalAlpha = a;
    ctx.translate(POLA.x + POLA.w / 2, POLA.y + POLA.h / 2);
    ...
    for (let i = 0; i < 70; i++) {
      ...
      ctx.beginPath();
      ctx.arc(PHOTO.x + seedX, PHOTO.y + seedY, 2.2, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.restore();

    // eyebrow label
    const p0 = progressBetween(t, 0.12, 0.28);
    if (p0 > 0) {
      ctx.save();
    ...
    // brand title
    const p2 = progressBetween(t, 0.36, 0.52);
    if (p2 > 0) {
      ctx.save();
    ...
    // name
    const p3 = progressBetween(t, 0.48, 0.64);
    if (p3 > 0) {
      ctx.save();
    ...
    // date/time
    const p4 = progressBetween(t, 0.62, 0.76);
    if (p4 > 0) {
